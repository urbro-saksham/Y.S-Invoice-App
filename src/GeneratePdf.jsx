...
import sendEmailNotification from "./JS/SendEmail.js";
...
export default function GeneratePDF() {
  ...
  const totals = useMemo(() => {
    ...
    // taxable total — use taxableAmount if entered, else fallback to amount
    const taxableTotal = items.reduce(
      (a, b) =>
        a + (Number(b.taxableAmount) || Number(b.amount) || 0),
      0
    );

    // calculate GST based on taxable value
    const tax = isIgst
      ? (taxableTotal * igstPercent) / 100
      : ((taxableTotal * cgstPercent) / 100) +
        ((taxableTotal * sgstPercent) / 100);

    const total = subTotal + tax;
    return { subTotal, tax, total, taxableTotal };
  }, [items, isIgst, igstPercent, cgstPercent, sgstPercent]);
...
const onDownload = async () => {
  ...
  try {
    ...
    // capture the invoice as image (for PDF)
    ...
    const invoiceNo = watch("invoiceNo") || "Draft";
    const items = watch("items") || [];
    const total = totals.total || 0;

    // Send email via EmailJS
    sendEmailNotification(invoiceNo, total, items);
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <form className="card p-4 lg:p-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="min-w-48 text-right">
            <span className="muted">Taxable Total:</span> ₹ {currency(totals.taxableTotal)}
          </div>
          <div className="min-w-48 text-right">
